...
def load_model(checkpoint_path: str) -> torch.nn.Module:
    """Loads HCTG-Net with the specified checkpoint."""
    try:
...
def run_inference(model: torch.nn.Module, tensor: torch.Tensor) -> tuple:
    ...
    # Ensure probs always has 5 values even if model
    # was trained on fewer classes
    if len(probs) < 5:
...
4. The model will classify the beat and display the diagnosis.
        ...
        model_choice = st.selectbox(
            ...
            help="MIT-BIH: trained on 47 patients. PTB-XL: trained on 18,885 patients."
        )

        # Set checkpoint based on selection
        if model_choice == "MIT-BIH Trained":
            selected_checkpoint = 'best_hctg_net.pth'
            dataset_info = "MIT-BIH Arrhythmia Database (47 patients, 109,466 beats)"
            model_acc  = "98.82%"
            model_f1   = "93.61%"
            model_auc  = "0.9869"
        else:
            selected_checkpoint = 'results/ptbxl/best_ptbxl_model.pth'
            dataset_info = "PTB-XL Database (18,885 patients, 21,799 records)"
            model_acc  = "92.62%"
            model_f1   = "83.71%"
            model_auc  = "0.9798"
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("🔍 Analysing ECG beat..."):
            try:
                ...
                st.info("Try switching to MIT-BIH model if using PTB-XL trained model.")
                st.stop()
